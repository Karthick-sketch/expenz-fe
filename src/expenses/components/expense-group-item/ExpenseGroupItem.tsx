  ...
  return (
    <li>
      <div
        ...
        role="button"
        tabIndex={0}
        onKeyDown={(e) => e.key === "Enter" && navigate(`/expenses/groups/${id}`)}
      >
        {/* Icon */}
        ...
        <div className="group-main">
          ...
          <div className="group-meta">
            ...
            {incomeCount > 0 && `${incomeCount} income${incomeCount !== 1 ? "s" : ""}`}
          </div>
        </div>

        {/* Balance + sub-totals */}
        <div className="group-amounts">
          <span className={`group-balance ${isPositive ? "is-income" : "is-expense"}`}>
            {isPositive ? "+" : "-"}{fmt(balanceAmount)}
          </span>
          <div className="group-sub-amounts">
            {totalExpensesAmount > 0 && (
              <span className="group-sub expense">-{fmt(totalExpensesAmount)}</span>
            )}
            {totalIncomesAmount > 0 && (
              <span className="group-sub income">+{fmt(totalIncomesAmount)}</span>
            )}
          </div>
        </div>
